...
function createQueueThat(options) {
  ...
  options.maxBackoffTime = options.maxBackoffTime || MAX_BACKOFF_TIME;
  ...
  if (options.backoffTime > options.maxBackoffTime) {
    throw new Error('max backoff time must be greater than backoff time');
  }
  ...
  function checkQueue() {
    ...
    processTimer = setTimeout(function() {
      ...
      timeout = true;
      processingTasks = false;
      if (options.onProcessTimeout) options.onProcessTimeout(batch);
  ...
  function processError(err) {
    ...
    storageAdapter.setBackoffTime(
      now() + Math.min(options.backoffTime * Math.pow(2, errorCount - 1), options.maxBackoffTime)
    );
  ...
  /**
   * Deactivating the queue on unload is not
  ...
  function deactivateOnUnload(queueId) {
    if (window.addEventListener) {
      ...
      window.attachEvent('onunload', deactivate);
    }
